...
# Loading functions
functions.fn <- 'scripts/pseudobulkDEA_inrt_lmer_functions.R'
...
# opt$in_dir <- '/gpfs/projects/bsc83/Projects/scRNAseq/aripol1/sc-eqtlgen-consortium-pipeline/ongoing/wg3/wg3_wijst2018/input'
# opt$out_dir <- '/gpfs/projects/bsc83/Projects/scRNAseq/aripol1/sc-eqtlgen-consortium-pipeline/ongoing/wg3-sc_pseudobulk_DEA-updated/wg3_wijst2018/pseudobulkDEA_inrt_lmer'

## okada
# opt$cell_type <- 'B'
# opt$in_dir <- '/gpfs/projects/bsc83/Projects/scRNAseq/aripol1/sc-eqtlgen-consortium-pipeline/ongoing/wg3/wg3_okada/input'
# opt$covariates <- 'pseudobulkDEA_dreamlet.covariates.okada.tab'
# opt$out_dir <- 'pseudobulkDEA_inrt_lmer.okada'
# opt$covariates <- 'pseudobulkDEA_dreamlet.covariates.test.tab'
# opt$out_dir <- 'pseudobulkDEA_inrt_lmer.okada.test'

## Li
# opt$cell_type <- 'B'
# opt$in_dir <- '/gpfs/projects/bsc83/Projects/scRNAseq/aripol1/sc-eqtlgen-consortium-pipeline/ongoing/wg3/wg3_Li.addMetadata/input'
# opt$out_dir <- 'pseudobulkDEA_inrt_lmer.Li'
# opt$covariates <- 'pseudobulkDEA_dreamlet.covariates.Li.tab'

## Nawijn_samplecondition
# opt$cell_type <- 'B'
# opt$in_dir <- '/gpfs/projects/bsc83/Projects/scRNAseq/aripol1/sc-eqtlgen-consortium-pipeline/ongoing/wg3/wg3_Nawijn.addMetadata.sample_condition/input'
# opt$out_dir <- 'pseudobulkDEA_inrt_lmer.Nawijn_samplecondition'

## Ye
# opt$cell_type <- 'B'
# opt$in_dir <- '/gpfs/projects/bsc83/Projects/scRNAseq/aripol1/sc-eqtlgen-consortium-pipeline/ongoing/wg3/wg3_Ye.addMetadata/input'
# opt$out_dir <- 'pseudobulkDEA_inrt_lmer.Ye'
# opt$genoPC1 <- TRUE
...
out.dir <- paste0(out.dir, '/', opt$interaction, '/')
...
print(paste0('nCells per donor: ', opt$nCells_per_donor))
print(paste0('Interaction: ', opt$interaction))
print(paste0('Covariates file: ', opt$covariates))
print(paste0('Min prop: ', opt$min_prop))
print(paste0('VariancePartition reduced: ', opt$vp_reduced))
print(paste0('Combined data: ', opt$combined_data))
print(paste0('genoPC1: ', opt$genoPC1))
print(paste0('Filter replicates: ', opt$filter_replicates))
...
# Filtering: processAssays (genes & samples)
system.time(process_data.list <- process_data(ge_dge = pb, 
                                              covariates = covs.df, 
...
#   sapply(genes_expressed, function(j) lmer_func(g = j, 
#                                                 norm = i,
#                                                 process_data_list = process_data.list, 
#                                                 phenotype = opt$phenotype), simplify = FALSE), simplify = FALSE)) #to debug

print('Fitting lmer...')  
system.time(lmer_out <- setNames(
    mclapply(norm_methods, function(i){
      setNames(
        mclapply(genes_expressed, function(j) lmer_func(g = j, 
                                                        norm = i, 
                                                        process_data_list = process_data.list, 
...
if(opt$phenotype%in%c('SEX','age_cat', 'age_cat_all')){
  Group_order <- c('M','F')
